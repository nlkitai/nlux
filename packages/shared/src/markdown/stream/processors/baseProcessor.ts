...
export abstract class BaseMarkdownProcessor
  implements MarkdownProcessorInterface
{
   ...
   */
  abstract createAndAppendMarkdown(
    elementName: MarkdownElementName,
  ...
  public yield(
    ...
    const openingSequence =
      this.__sequenceParser &&
      !this.__sequenceParser?.shouldCloseCurrentMarkdown
        ? this.__sequenceParser?.sequence
        : undefined;
    ...
    if (this.__element) {
      const sanitizedHTML = sanitizeHTML(this.__element.innerHTML.trim());
      this.__element.innerHTML = sanitizedHTML;
