...
```tsx
useStreamAdapter<AiMsg = string>(submit: StreamSubmit)
```
...
```tsx
export type StreamSubmit<AiMsg> = (
    message: string,
...
When implementing your `submit`, use the `observer.next(chunk)` method when data is received from the API.<br />
