...
export abstract class ProcessorWithChildren extends BaseMarkdownProcessor {
  protected constructor(
    ...
    super(parent, elementName, openingSequence, initialContent, options);
  }
  ...
  processCharacter(character: string): void {
    ...
    if (
      ...
      if (this.domElement?.innerHTML) {
        const sanitizedHTML = sanitizeHTML(this.domElement?.innerHTML.trim());
        this.domElement.innerHTML = sanitizedHTML;
      }
