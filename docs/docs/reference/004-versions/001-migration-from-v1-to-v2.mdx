...
`NLUX v2.x` was released in May 2024, with several breaking changes.<br />
If you're a user of `NLUX` `v1` looking to migrate to `v2.x`, this guide is for you.
...
<br />
<br />
The `NLUX` `v1`'s default theme is called `Luna` and it's still included as part of the `v2.x` release, but not as a
