...
export class CompList<CompType extends BaseComp<any, any, any, any>>
    extends BaseComp<CompListProps, CompListElements, CompListEvents> {
    ...
    constructor(instanceId: string, props: CompListProps) {
    ...
    public appendComponent(
        ...
        requestAnimationFrame(() => {
            ...
            itemContainer.dataset.nluxId = componentToAppend.id;
    ...
    public removeComponent(component: BaseComp<any, any, any>) {
    ...
    public render(root: HTMLElement | DocumentFragment) {
        ...
        if (!this.renderedDom) {
            ...
            const renderedDom = this.executeRenderer(root, this.id);
            if (!renderedDom) {
                throw new NluxError({
                    source: this.constructor.name,
        ...
        if (this.componentsToRender.length > 0) {
            ...
            for (const item of this.componentsToRender) {
                ...
                const itemContainer = document.createElement('div');
                itemContainer.dataset.nluxId = component.id;
                if (containerCssClass) {
                    itemContainer.className = containerCssClass;
                }
