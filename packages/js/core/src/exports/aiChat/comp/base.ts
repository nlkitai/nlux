import {
  CallbackArgType,
  CallbackFunction,
} from '../../../../../../shared/src/types/callbackFunction';
import {
  NluxError,
  NluxUsageError,
...
export abstract class BaseComp<
  ...
  ActionsType
> {
  static __compEventListeners: Map<string | number | symbol, string[]> | null =
    null;
  static __compId: string | null = null;
  static __renderer: CompRenderer<unknown, unknown, unknown, unknown> | null =
    null;
   ...
   */
  protected readonly def: CompDef<
    PropsType,
    ElementsType,
    EventsType,
    ActionsType
   ...
   */
  protected subComponents: Map<
    string,
    BaseComp<unknown, unknown, unknown, unknown, unknown>
  > = new Map();
  ...
  private compEventGetter = (eventName: EventsType) => {
    ...
    if (!callback) {
      throw new NluxError({
        ...
        message:
          `Unable to call renderer event "${eventName}" because no matching event listener was found. ` +
  ...
  protected constructor(context: ControllerContext<AiMsg>, props: PropsType) {
    ...
    if (!compId) {
      throw new NluxUsageError({
        source: this.constructor.name,
        message:
          'Unable to instantiate component: missing compId in implementation. ' +
    ...
    if (!this.def) {
      throw new NluxUsageError({
        source: this.constructor.name,
        message:
          `Unable to instantiate component "${compId}" because it\'s not registered. ` +
    ...
    const preDefinedEventListeners: Map<string, string[]> | null = (<any>(
      this.constructor
    )).__compEventListeners;
      ...
      preDefinedEventListeners.forEach((methodNames, eventName) => {
        methodNames.forEach((methodName) => {
          ...
          if (typeof method === 'function') {
            this.addRendererEventListener(
              eventName as EventsType,
              method.bind(this)
            );
          } else {
            warn(
              `Unable to set event listener "${eventName}" because method "${methodName}" ` +
  ...
  public render(root: HTMLElement, insertBeforeElement?: HTMLElement) {
    ...
    if (this.destroyed) {
      warn(
        `Unable to render component "${this.def?.id}" because it is already destroyed`
      );
      return;
    }
  ...
  updateSubComponent(
    ...
    const subComp = this.subComponents.get(subComponentId) as
      | BaseComp<AiMsg, PropsType, ElementsType, EventsType, ActionsType>
      | undefined;

    if (subComp && !subComp.destroyed) {
      subComp.setProp(
        propName as keyof PropsType,
        newValue as PropsType[keyof PropsType]
      );
    }
  }

  protected addSubComponent<
    SubCompPropsType,
    SubCompElementsType,
    SubCompEventsType,
    SubCompActionsType
  >(
    ...
    this.subComponents.set(
      id,
      subComponent as BaseComp<unknown, unknown, unknown, unknown, unknown>
    );
   ...
   * @protected
   */
  protected executeDomAction(
    actionName: keyof ActionsType,
    ...args: Array<unknown>
  ) {
    this.throwIfDestroyed();

    if (!this.renderedDom) {
      this.actionsOnDomReady.push(() =>
        this.executeDomAction(actionName, ...args)
      );
      return;
    }
    ...
    return domOp(() =>
      (action as CallbackFunction)(...(args as CallbackArgType[]))
    );
  }
  ...
  protected setProp(name: keyof PropsType, value: PropsType[keyof PropsType]) {
    if (this.destroyed) {
      warn(
        `Unable to set prop "${String(name)}" because component "${
    ...
    this.schedulePropUpdate(name, this.elementProps.get(name)!, value);

    this.props = Object.freeze(
      Object.fromEntries(this.elementProps)
    ) as Readonly<PropsType>;
  ...
  private addRendererEventListener(
      ...
      domOp(() => {
        ...
        if (renderingRoot instanceof DocumentFragment) {
          ...
          if (isListItem) {
            ...
            renderingRoot.innerHTML = sanitizedHTML;
          }
        }
      });
  ...
  private mountSubComponentToPortal(
    subComponentId: string,
    portal: HTMLElement
  ) {
