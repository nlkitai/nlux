...
export class CodeBlock extends ProcessorWithChildren {
    ...
    createElement(): HTMLElement {
        ...
        if (this.language) {
            element.dataset.language = this.language;
        }
    ...
    processCharacter(character: string): void {
        ...
        if (sequenceDiscarded) {
            ...
            if (secondCheck.sequenceDiscarded) {
                ...
                if (discardedSequence && this.domElement) {
                    // Append the discarded sequence to the DOM, character by character
                    for (const character of discardedSequence) {
                        if (!this.domElement.innerHTML && character === '\n') {
                            // We don't want to append new line as the first child
                            continue;
                        }

                        this.domElement.append(character);
                    }
                }
            } else {
    ...
    yield(elementToCreateAtParentLevel?: MarkdownElementName, characterToAppendToParent?: string) {
        // Apply syntax highlighting before yielding
        // We attempt to retrieve the code via innerText first, and fallback to innerHTML
        const innerContent = this.domElement?.innerText ?? this.domElement?.innerHTML;
        if (this.syntaxHighlighter && this.domElement && innerContent) {
            const language = this.domElement.dataset.language;
            if (language) {
                ...
                this.domElement.classList.add(this.syntaxHighlighter.highlightingClass(language));
                this.domElement.innerHTML = highlight(innerContent, language);
            }
        }
    ...
    private insertCopyToClipboardButton() {
        ...
        copyButton.addEventListener('click', () => {
            if (clicked || !domElement) {
                return;
            }

            // Copy code to clipboard
            const code = domElement.innerText;
