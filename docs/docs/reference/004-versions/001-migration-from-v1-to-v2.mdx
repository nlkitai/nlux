...
<br />
<br />
`NLUX` `v1` was released in January 2024.<br />
`NLUX` `v2` was released in May 2024, with several breaking changes.<br />
If you're a user of `NLUX` `v1` looking to migrate to `v2`, this guide is for you.

Scope of the breaking changes:

* `Theming system` and default theme
* Several TypeScript `type changes`
* `AiChat` UI component config options
...
<br />
<br />
NLUX v2 comes with a new default theme called `Nova`.<br />
...
We recommend migrating to the `Nova` theme, as it's a more user-friendly accessible theme, that supports both
dark and light modes. But if you would like to keep the old theme, you have that option.

### Keeping the v1 theme

<br />
<br />
The `NLUX` `v1`'s default theme is called `Luna` and it's still included as part of the `v2` release, but not as a
default theme. If you're interested in keeping the `Luna` theme while using `NLUX` `v2`, you can that by providing
...
`NLUX` `v2` has updated the name and value of certain types. Here's a list of the updated types:
...
<br />
<br />
`AiChat` UI component's config options have been updated in `NLUX` `v2`.<br />
...
| `promptOptions.autoFocus` | `componentOptions.autoFocus` | `promptOptions.autoFocus` has been moved to `componentOptions`. |
| `promptOptions.placeholder` | `componentOptions.placeholder` | `promptOptions.placeholder` has been moved to `componentOptions`. |
...
[`NLUX` `v2` AI Chat component documentation](/reference/ui/ai-chat) for a complete list of changes.
...
If you have implemented custom adapters in your project, you will need to update them to work with `NLUX` `v2`.<br />
...
Please refer to the [`NLUX` `v2` custom adapters documentation](/reference/adapters) for more information on how to
