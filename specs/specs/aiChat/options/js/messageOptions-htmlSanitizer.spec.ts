...
describe('createAiChat() + messageOptions + htmlSanitizer', () => {
    ...
    describe('When htmlSanitizer is set to a custom function', () => {
        it('Renderer user prompt content should be sanitized using the custom function', async () => {
            ...
            // Assert
            const userMessageContainer = rootElement.querySelector('.nlux_cht_itm_out .nlux-md-cntr') as HTMLElement | null;
        ...
        it('Renderer ai message content should be sanitized using the custom function', async () => {
            ...
            // Assert
            const markdownContainer = rootElement.querySelector('.nlux_cht_itm_in .nlux-md-cntr') as HTMLElement | null;
            expect(markdownContainer).toBeInTheDocument();
            expect(markdownContainer!.innerHTML).toBe('<div>xi! <a xref="xttps://xaxa.xawai" target="_blank">xere</a> i xand over a link! xaxa :) !\n</div>');
        });
    })
});
