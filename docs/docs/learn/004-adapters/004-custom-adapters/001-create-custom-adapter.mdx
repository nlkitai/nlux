...
:::note
If you're using the React version of `NLUX`, you can use the `useStreamingAdapter` hook to create a streaming adapter
as shown below:
...
:::note
If you're using the React version of `NLUX`, you can use the `useBatchAdapter` hook to create a streaming adapter
as shown below:
...
```typescript
import {useBatchAdapter, AdapterExtras} from '@nlux/react';

export const myCustomAdapter = useBatchAdapter(
